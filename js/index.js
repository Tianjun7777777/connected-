 ...
 * And a webBLE example from Tom Igoe
 * https://tigoe.github.io/BluetoothLE-Examples/web-bluetooth/
...
const handleUseInputValue = (glScene) => {
  ...
  // const searchValue = document.getElementById('searchTextValue');
...
function initSocketConnection() {
  ...
  socket.on("userDisconnected", (clientCount, _id, _ids) => {
    ...
    //operator should be clients._id or clients[_id];
      delete clients[_id];
      console.log("Now we have thosed users connected: "+Object.keys(clients));
    }
  });
  ...
  socket.on("userPositions", (_clientProps) => {
    ...
    // glScene.clientDist(_clientProps);
  });
  ...
  //////////////////interaction in three.js scene////////////////////
  socket.on("text",(data)=>{
    // console.log(data);
    glScene.createFont(data);
    console.log("createFont: "+data);
  });

  socket.on("mark",(data)=>{
    // console.log(data);
    glScene.createMark(data);
  });

  socket.on("trace",(data)=>{
    // console.log(data);
    glScene.createTrace(data);
  });

  socket.on("collideMark",(data)=>{
    glScene.createCollideMark(data);
  });
...
function createClientMediaElements(_id) {
  ...
  audioEl.volume = 1;
  document.getElementById("video_container").appendChild(audioEl);
